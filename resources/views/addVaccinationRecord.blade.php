...
@section('content')
    ...
    <script>
        document.getElementById('cageID').addEventListener('change', function() {
            ...
            // Fetch breeds associated with the selected cage and ensure uniqueness
            const uniqueBreeds = new Set();
            @foreach ($cages as $cage)
                if (selectedCageID == '{{ $cage->cageID }}') {
                    @foreach ($cage->chickens as $chicken)
                        if (!uniqueBreeds.has('{{ $chicken->breed->breedID }}')) {
                            uniqueBreeds.add('{{ $chicken->breed->breedID }}');
                            breedSelect.innerHTML +=
                                `<option value="{{ $chicken->breed->breedID }}">{{ $chicken->breed->name }}</option>`;
                        }
                    @endforeach
                }
            @endforeach
