...
def predict_class(img_path, model):
    ...
    predicted_class = int(np.argmax(predictions))  # Get the class with the highest score
...
def process_image(image_path):
    ...
    if last_processed_time is None or current_time - last_processed_time > processing_interval or image_hash != last_processed_time[1]:
        try:
            ...
            # Log the class ID and return the result
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        sys.stderr.write("Usage: python classifyV1DecEggWeightDetect.py <image_path>\n")
        sys.exit(1)
